...
def _get_default_edm_root(env_root=_get_default_environment_root()):
    if "EDM_VIRTUAL_ENV" in os.environ:
        root = abspath(join(os.environ["EDM_VIRTUAL_ENV"], "..", ".."))
    else:
...
@click.option("--edm-root", default=_get_default_edm_root(), help="EDM root")
@click.option("--edm-env", default="prod", help="project environment to use")
@click.option("--edm-py-version", default="3.8", help="Version of python to use")
@click.option("--edm-bin", default=_get_default_edm_bin(), help="EDM binary to use")
@click.pass_context
...
@root.command()
@click.option(
    "--replace/--no-replace", default=False, help="Force replace environment?"
)
@click.pass_obj
    ...
    try:
        subprocess.check_call(
            [
                ...
                "create",
                obj.edm_env,
                "--version={}".format(obj.edm_py_version),
            ]
            + args
        )
...
def install_deps(obj, source_dir):
    ...
    subprocess.check_call(cmd_base + ["install", "-e", obj.edm_env, "-y"] + ENV_DEPS)

    # install other requirements via pip
    edm_run_cmd = cmd_base + ["run", "-e", obj.edm_env, "--"]
    req = join(source_dir, "requirements.txt")
    subprocess.check_call(edm_run_cmd + ["pip", "install", "-r", req])
...
def migrate(obj):
    ...
    # db revision.
    subprocess.check_call(
        edm_run_cmd + ["flask", "--app", "txwtf.webapp", "db", "upgrade"],
...
def flake8(obj, root_cmd):
    ...
    edm_run_cmd = [obj.edm_bin, "-r", obj.edm_root, "run", "-e", obj.edm_env, "--"]
    subprocess.check_call(
        edm_run_cmd + [root_cmd, "flake8"], cwd=join(dirname(__file__), "..")
    )
...
def run_app(obj, root_cmd, log_file, log_level, profiling, cmd_args):
    ...
    """
    run_cmd = [obj.edm_bin, "-r", obj.edm_root, "run", "-e", obj.edm_env, "--"]
...
@click.option(
    "--access-logfile",
    envvar="WSGI_ACCESS_LOGFILE",
    default="-",
    help="Log file. Use '-' for stdout.",
)
@click.option(
    "--access-logformat",
    envvar="WSGI_ACCESS_LOGFORMAT",
    default='%(h)s %(l)s %(u)s %(t)s "%(r)s" %(s)s %(b)s "%(f)s" "%(a)s"',
    help="Log format for access log",
)
@click.option(
    "--error-logfile",
    envvar="WSGI_ERROR_LOGFILE",
    default="-",
    help="Error log file. Use '-' for stdout.",
)
@click.option(
    "--log-level", envvar="WSGI_LOG_LEVEL", default="info", help="Error log level."
)
@click.option(
...
def shell(obj):
    ...
    """
    edm_run_cmd = [obj.edm_bin, "-r", obj.edm_root, "shell", "-e", obj.edm_env]
    subprocess.check_call(edm_run_cmd)
...
def jobs(obj, keepalive, root_cmd, jobs_args):
    ...
    def _proc_mon(obj, cmd, keepalive):
        ...
        while True:
            ...
            print(
                "Running pid {} (keepalive: {}): {}".format(p.pid, keepalive, full_cmd)
            )
            while True:
                try:
