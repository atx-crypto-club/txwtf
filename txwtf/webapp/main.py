...
def editprofile():
    changes = (
        db.session.query(UserChange)
        .filter(UserChange.user_id == current_user.id)
        .order_by(UserChange.change_time.desc())
    )
    return render_template("editprofile.html", changes=changes)
...
def generate_render_post_data(dbposts):
    ...
    for dbpost in dbposts:
        ...
        post.num_reposts = len(
            db.session.query(PostedMessage)
            .filter(PostedMessage.repost_id == dbpost.id)
            .all()
        )
        post.replies = generate_render_post_data(
            db.session.query(PostedMessage)
            .filter(PostedMessage.reply_to == dbpost.id)
            .all()
        )
        ...
        post.reactions = (
            ...
            .filter(Reaction.post_id == dbpost.id, Reaction.deleted == False)
            .all()
        )  # noqa: E712
...
def increment_posts_view_count(posts):
    ...
    for msg in db.session.query(PostedMessage).filter(
        ...
        if current_user_id is not None and current_user_id == msg.user_id:
            continue
        ...
        db.session.add(pmv)
    for user in db.session.query(User).filter(User.id.in_(user_counts.keys())):
...
def user_view(username):
    ...
    if view == "public":
        dbposts = (
            db.session.query(PostedMessage)
            .filter(PostedMessage.user_id == user.id, PostedMessage.reply_to == None)
        ...
        dbposts = (
            db.session.query(PostedMessage)
            .join(Mention, PostedMessage.id == Mention.post_id)
            .filter(Mention.user_id == user.id)
            .order_by(PostedMessage.post_time.desc())
            .all()
        )
    ...
    db.session.commit()
    return render_template("users.html", user=user, posts=posts, view=view)
...
def user_list():
    return render_template(
        "userlist.html",
        users=db.session.query(User).order_by(User.post_view_count.desc()).all(),
    )
...
def posts():
    ...
    posts = generate_render_post_data(dbposts)
    dbtags = db.session.query(Tag).order_by(Tag.last_used_time.desc()).all()
    tags = []
...
def post_view(post_id):
    dbposts = (
        db.session.query(PostedMessage).filter(PostedMessage.id == int(post_id)).all()
    )
    if len(dbposts) == 0:
        return render_template("error.html", error_msg="Unknown post!")
    posts = generate_render_post_data(dbposts)
    dbreplies = (
        db.session.query(PostedMessage)
        .filter(PostedMessage.reply_to == int(post_id))
        .order_by(PostedMessage.post_time.asc())
        .all()
    )
    replies = generate_render_post_data(dbreplies)
    dbreposts = (
        db.session.query(PostedMessage)
        .filter(PostedMessage.repost_id == int(post_id))
        .order_by(PostedMessage.post_time.asc())
        .all()
    )
    ...
    db.session.commit()
    return render_template(
        "post_view.html", posts=posts, replies=replies, reposts=reposts
    )
...
def hash_tag_view(name):
    ...
    if not dbtag:
        return render_template("error.html", error_msg="Unknown hash tag!")
    dbposts = (
        db.session.query(PostedMessage)
        .join(HashTag, PostedMessage.id == HashTag.post_id)
        .filter(HashTag.tag_id == dbtag.id)
        .order_by(PostedMessage.post_time.desc())
        .all()
    )
    posts = generate_render_post_data(dbposts)
    ...
    return render_template(
        "post_view.html", posts=posts, title="#{}".format(name), description=description
    )
...
def scrape_mentions(content):
    ...
    for i in textList:
        if i[0] == "@":
            ...
            user = db.session.query(User).filter(User.username == mention).first()
            if user is None:
...
def add_reaction(user_id, post_id, reaction_name):
    ...
    if not emoji:
        ...
        new_log = SystemLog(
            ...
            event_time=now,
            event_desc="User {} Adding emoji {}".format(user.name, reaction_name),
            referrer=request.referrer,
            user_agent=str(request.user_agent),
            remote_addr=remote_addr(request),
            endpoint=request.endpoint,
    ...
    log_reaction = UserChange(
        ...
        change_code=31337,
        change_time=now,
        change_desc="Adding {} reaction to post {}".format(reaction_name, post_id),
        referrer=request.referrer,
        user_agent=str(request.user_agent),
        remote_addr=remote_addr(request),
        endpoint=request.endpoint,
...
def remove_reaction(user_id, post_id, reaction_name):
    ...
    if not reaction:
        logger.warning(
            "No reaction {} by user {} for post {}".format(
    ...
    log_reaction = UserChange(
        ...
        change_desc="Removing {} reaction to post {}".format(reaction_name, post_id),
        referrer=request.referrer,
        user_agent=str(request.user_agent),
        remote_addr=remote_addr(request),
        endpoint=request.endpoint,
...
def post_message():
    ...
    for username, _ in mentions:
        markdown_content = markdown_content.replace(
            "@{}".format(username), "[@{}](/u/{})".format(username, username)
        )
    ...
    for hashtag in hashtags:
        ...
        new_hashtag = HashTag(post_id=msg.id, tag_id=dbtag.id, post_time=now)
        db.session.add(new_hashtag)

    for username, user_id in mentions:
        new_ment = Mention(user_id=user_id, post_id=msg.id)
...
def delete_post():
    post_id = request.form.get("post_id")
    post = (
        db.session.query(PostedMessage).filter(PostedMessage.id == int(post_id)).first()
    )
    if post.user_id != current_user.id:
        logger.error(
            "Unauthorized post delete: {} {}".format(post.user_id, current_user.id)
        )
        return render_template("error.html", error_msg="Unauthorized post delete"), 401
    post.deleted = True
    ...
    new_log = SystemLog(
        ...
        event_desc="User {} deleted post {}".format(current_user.email, post.id),
        referrer=request.referrer,
        user_agent=str(request.user_agent),
        remote_addr=remote_addr(request),
        endpoint=request.endpoint,
...
def upload_avatar():
    if "avatar" in request.files:
        ...
        saved_name = upload_archive.save(
            request.files["avatar"], folder=str(current_user.username)
        )
        ...
        logger.info(
            "Changing user {} avatar image to: {}".format(
...
def upload_header_image():
    if "header_image" in request.files:
        if request.files["header_image"].filename == "":
            ...
            return redirect(url_for("main.editprofile"))
        saved_name = upload_archive.save(
            request.files["header_image"], folder=str(current_user.username)
        )
        ...
        logger.info(
            "Changing user {} header image to: {}".format(
                current_user.email, saved_name
            )
        )
        return redirect(url_for("main.editprofile"))
...
def upload_card_image():
    if "card_image" in request.files:
        if request.files["card_image"].filename == "":
            ...
            return redirect(url_for("main.editprofile"))
        saved_name = upload_archive.save(
            request.files["card_image"], folder=str(current_user.username)
        )
        current_user.card_image_url = "/uploads/{}".format(saved_name)
        current_user.modified_time = datetime.now()
        new_change = UserChange(
            user_id=current_user.id,
            change_code=31337,  # default for now
        ...
        db.session.add(new_log)
        db.session.commit()
        flash("Card image saved successfully as {}.".format(saved_name))
        logger.info(
            "Changing user {} card image to: {}".format(current_user.email, saved_name)
        )
        return redirect(url_for("main.editprofile"))
    else:
        flash("Invalid request")
...
def update_user_description():
    ...
    for hashtag in scrape_hashtags(desc):
        dbht = db.session.query(Tag).filter(Tag.name == hashtag).first()
        if dbht is None:
    ...
    db.session.add(new_change)
    db.session.commit()
    logger.info("Changing user {} description to: {}".format(current_user.email, desc))
    return redirect(url_for("main.editprofile"))
...
def update_user_name():
    ...
    db.session.add(new_change)
    db.session.commit()
    logger.info("Changing user {} name to: {}".format(current_user.email, name))
    return redirect(url_for("main.editprofile"))
...
def update_user_header_text():
    ...
    db.session.commit()
    logger.info(
        "Changing user {} header text to: {}".format(current_user.email, header_text)
    )
    return redirect(url_for("main.editprofile"))
...
def uploads(path):
    return send_from_directory(current_app.config["UPLOADED_ARCHIVE_DEST"], path)
...
def upload_file():
    ...
    if "user_file" in request.files:
        ...
        saved_name = upload_archive.save(
            request.files["user_file"], folder=str(current_user.username)
        )
        user_file = UserFile(
            name=name,
